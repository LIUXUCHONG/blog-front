...
const add = async () => {
    let res = await axios.post("/api/admin/category", { name: addCategory.name })
...
const update = async ()=>{
    console.log("我是id",updateCategory.id,updateCategory.name)
    let res = await axios.put("/api/admin/category", {
...
const deleteCategory = async (category) => {
    dialog.warning({
        ...
        onPositiveClick: async () => {
            let res = await axios.delete(`/api/admin/category/${category.ID}`)
            if (res.data.code == 200) {
                loadDatas()
                message.info(res.data.msg)
            } else {
